...
def add_kudos(db: Session, kudos: Kudos):
    try:
        if not check_kudos_exists(db, kudos.post_id, kudos.user_id):
            raise HTTPException(
                status_code=404, detail=f"Kudos not found for post with ID {kudos.post_id} and user with ID {kudos.user_id}"
            )
